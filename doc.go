...
A typical use case of sorted set is a leaderboard in a massive online game, where you update the player's position every time a new score is submitted. You can easily take the top players using GetByRankRange() method. You can also, given an user id, return their rank in the listing using FindRank() method. Using FindRank() and GetByRankRange() together you can show users with a score similar to a given user.
